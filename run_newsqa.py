...
from DataLoader import NewsqaExample, NewsqaLoader
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False):
    ...
    # (the NewsQA eval script also does punctuation stripping/lower casing but
...
def main():
    ...
    if args.do_test:
        if not args.predict_file:
            raise ValueError(
                "If `do_predict` is True, then `te_file` must be specified.")
    ...
    newsqaloader = NewsqaLoader() # NewsQA的数据加载以及提炼三元组
    if args.do_train:
        print("reading train...")
        # 读取训练集，若本地不存在，则重新加载
        train_example_file = os.path.join(args.newsqa_data_dir, 'newsqa_train_example_file.pkl')
        if os.path.exists(train_example_file):
            ...
            train_examples = newsqaloader.read_newsqa_examples(status='train')
    ...
    # output_model_file = os.path.join(args.output_dir, "newsqa_epoch_0_pytorch_model.bin")
    ...
    if args.do_train:
        ...
        if os.path.exists(cached_train_features_file):
            ...
            train_features = newsqaloader.convert_examples_to_features(
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ncols=50)):
                ...
                input_kg_path_mask, entity2id_list = \
                    newsqaloader.find_shortest_path_and_convert_to_tensor(bert_refine_triples_ids,
                                                                         bert_refine_triples_with_cls_ids,
                                                                         entity_ids, entpair2tripleid, cls_id)

                input_token_to_entity, input_path_mask = newsqaloader.get_tokens_to_entity(bert_refine_triples_with_cls_ids, tokens_to_triple, entity2id_list)
                ...
                if (step + 1) % save_model_every == 0:
                    ...
                    output_model_file = os.path.join(args.output_dir,
                                                     "newsqa_epoch_" + str(epoch) + "_pytorch_model.bin")
                    torch.save(model_to_save.state_dict(), output_model_file)
                    np.save(os.path.join(args.output_dir, "newsqa_epoch_" + str(epoch) + "_loss_trend.bin"), loss_trend)
            ...
            # Save a trained model
            model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
            output_model_file = os.path.join(args.output_dir, "newsqa_epoch_" + str(epoch) + "_pytorch_model.bin")
            torch.save(model_to_save.state_dict(), output_model_file)
            np.save(os.path.join(args.output_dir, "newsqa_epoch_" + str(epoch) + "_loss_trend.bin"), loss_trend)

        with open(os.path.join(args.output_dir, "newsqa_train_loss.pkl"), 'wb') as f:
            pickle.dump(TrainLoss, f)
    # 预测阶段
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        print("reading dev...")
        eval_example_file = os.path.join(args.newsqa_data_dir, 'newsqa_eval_example_file.pkl')
        cached_eval_features_file = os.path.join(args.newsqa_data_dir, 'newsqa_eval_features_file.pkl')
        ...
        if os.path.exists(eval_example_file):
            ...
            eval_examples = newsqaloader.read_newsqa_examples(status='dev')
        ...
        if os.path.exists(cached_eval_features_file):
            ...
            eval_features = newsqaloader.convert_examples_to_features(
        ...
        # with open(os.path.join(args.output_dir, "newsqa_train_loss.pkl"), 'rb') as f:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for batch in tqdm(eval_dataloader, desc="Evaluating",ncols=50):
                ...
                input_token_to_entity, input_path_mask = newsqaloader.get_tokens_to_entity(
            ...
            output_prediction_file = os.path.join(args.output_dir, "newsqa_epoch_" + str(epoch) + "_predictions.json")
            output_nbest_file = os.path.join(args.output_dir, "newsqa_epoch_" + str(epoch) + "_nbest_predictions.json")
            output_null_log_odds_file = os.path.join(args.output_dir, "newsqa_epoch_" + str(epoch) + "_null_odds.json")
...
def check_data():
    ...
    train_struct_file = os.path.join(args.newsqa_data_dir, 'newsqa_train_structure_knowledge.npy')
    dev_struct_file = os.path.join(args.newsqa_data_dir, 'newsqa_dev_structure_knowledge.npy')
    test_struct_file = os.path.join(args.newsqa_data_dir, 'newsqa_tests_structure_knowledge.npy')
    if not os.path.exists(args.train_file) or not os.path.exists(args.predict_file):
        raise FileExistsError(
            "You Need to download 'train-v2.0.json' and 'dev-v2.0.json' from newsqa website, and then put them into directory '/data/newsqa/'")

    if not os.path.exists(train_struct_file) or not os.path.exists(dev_struct_file):
        raise FileExistsError("You Need to generate 'newsqa_train_structure_knowledge.npy' and 'newsqa_dev_structure_knowledge.npy', you can refer to directory /utils/auto_extractor/readme.txt")
