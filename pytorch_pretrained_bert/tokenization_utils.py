...
class PreTrainedTokenizer(object):
    ...
    # 创建类对象后，可像调用属性一样执行方法，但不能带有括号，否则会报错（not callable）
    # 例如下面bos_token本是一个方法，加上@property可当做属性，用户可使用，但并不知道类的具体属性self._bos_token，避免被修改
    @property
    def bos_token(self):
    ...
    def bos_token_id(self):
        ...
        if self._bos_token is None:
            logger.error("Using bos_token, but it is not set yet.")
        return self.convert_tokens_to_ids(self._bos_token) # 根据字典将token转换为id或id序列
    ...
    def eos_token_id(self):
        ...
        if self._eos_token is None:
            logger.error("Using eos_token, but it is not set yet.")
        return self.convert_tokens_to_ids(self._eos_token) # 根据字典将token转换为id或id序列
    ...
    def unk_token_is(self):
        ...
        if self._unk_token is None:
            logger.error("Using unk_token, but it is not set yet.")
        return self.convert_tokens_to_ids(self._unk_token) # 根据字典将token转换为id或id序列
    ...
    def sep_token_id(self):
        ...
        if self._sep_token is None:
            logger.error("Using sep_token, but it is not set yet.")
        return self.convert_tokens_to_ids(self._sep_token) # 根据字典将token转换为id或id序列
    ...
    def pad_token_id(self):
        ...
        if self._pad_token is None:
            logger.error("Using pad_token, but it is not set yet.")
        return self.convert_tokens_to_ids(self._pad_token) # 根据字典将token转换为id或id序列
    ...
    def cls_token_id(self):
        ...
        if self._cls_token is None:
            logger.error("Using cls_token, but it is not set yet.")
        return self.convert_tokens_to_ids(self._cls_token) # 根据字典将token转换为id或id序列
    ...
    def mask_token_id(self):
        ...
        if self._mask_token is None:
            logger.error("Using mask_token, but it is not set yet.")
        return self.convert_tokens_to_ids(self._mask_token) # 根据字典将token转换为id或id序列
    ...
    def additional_special_tokens_ids(self):
        ...
        if self._additional_special_tokens is None:
            logger.error("Using additional_special_tokens, but it is not set yet.")
        return self.convert_tokens_to_ids(self._additional_special_tokens) # 根据字典将token转换为id或id序列

    def __init__(self, max_len=None, **kwargs):
        # **kwargs对应的一个dict字典，可以传入多个参数
        # 需要注意的是：尽管’kwargs’接收键值参数作为一个字典,但你不能传一个字典作为位置参数给’kwargs’
        ...
        for key, value in kwargs.items(): # 传入多值参数kwards，需要用字典来解析
            if key in self.SPECIAL_TOKENS_ATTRIBUTES:
                if key == 'additional_special_tokens':
                    assert isinstance(value, (list, tuple)) and all(isinstance(t, str) or (six.PY2 and isinstance(t, unicode)) for t in value)
                else:
                    assert isinstance(value, str) or (six.PY2 and isinstance(value, unicode))
                setattr(self, key, value) # 内置函数，功能：setattr(x, 'y', v) is equivalent to ``x.y = v''
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        if pretrained_model_name_or_path in s3_models:
            # Get the vocabulary from AWS S3 bucket 下载
    ...
    def _convert_token_to_id_with_added_voc(self, token):
        if token in self.added_tokens_encoder: # 当前的token在字典里，则直接返回对应的id
    ...
    def all_special_ids(self):
        """ List the vPRETRAINED_VOCAB_FILES_MAPocabulary indices of the special tokens ('<unk>', '<cls>'...) mapped to
