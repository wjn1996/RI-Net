...
from pytorch_pretrained_bert.knowledge_graph_inference import BertKGInferenceEncoder
import numpy as np
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, span_mask=None):
        all_encoder_layers = []
        ...
        if span_mask is not None:
            ...
            for layer_module in self.span_layer:
                span_hidden_states = layer_module(span_hidden_states, span_mask)
...
# 提出的新模型：Refine-then-Inference Model——Refine：先通过工具实现对文本自动抽取结构化知识作为辅助推理；Inference：再根据结构知识辅助推理
class BertInferenceQuestionAnswerSpanModule(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config) # bert模型
        self.span_layer = BertLayer(config) # 多层Transformer模型
        self.kg_inference_layer = BertKGInferenceEncoder(config))
        self.w = nn.Parameter(torch.Tensor([0.5, 0.5])) # 原文中对应的是alpha
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_rel_path=None, input_ent_path=None,
                start_positions=None, end_positions=None, is_impossibles=None):
        ...
        # 多层的transformer（直接使用bert自带的layer）
        # 这里的依存句法得到的mask相当于最初喂入BERT里的mask一样功能，在算attention时对被mask的部分不计算权重（或分配非常小的数）
        span_sequence_output = self.span_layer(self.bert_output, extended_span_attention_mask)

        w = F.softmax(self.w)
        # bert原始的输出（SAN）以及加入句法树解析的mask后的输出（Syntax-Guided SAN）进行加权求和
        sequence_output = self.gamma * (w[0] * self.bert_output + w[1] * span_sequence_output) # [batch_size, seq_len, hidden_size]
        # 下面的部分完全和BERT官方提供的问答的程序一样，对start和end进行预测
        logits = self.qa_outputs(sequence_output) # [batch_size, seq_len, 2]
        start_logits, end_logits = logits.split(1, dim=-1) # [batch_size, seq_len, 1]
        start_logits = start_logits.squeeze(-1) # [batch_size, seq_len]
