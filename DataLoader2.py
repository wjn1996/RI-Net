...
from utils.utils import get_equal_rate, int_list_to_str, str_to_int_list, find_Shortest_Path
...
class SquadLoader():
    ...
    def read_squad_examples(self, is_training=True):
        ...
        input_file = self.train_file if is_training else self.predict_file
        struct_file = self.train_struct_file if is_training else self.dev_struct_file
    ...
    def convert_examples_to_features(self, examples, tokenizer, max_seq_length,
        ...
        features = []
        flag = True
        save_every_time = 200 # 表示每处理一定数量的样本后存入磁盘，以避免内存溢出
        ...
        for (example_index, example) in enumerate(tqdm(examples, ncols=50, desc="generate features:")):
            ...
            ents_path_dict = find_Shortest_Path(bert_refine_triples_ids, bert_refine_triples_with_cls_ids, entity_ids, entpair2tripleid, cls_id)
            ...
            for (doc_span_index, doc_span) in enumerate(bert_passage_spans): # 遍历passage的每个chunk
                ...
                tokens_rel_path, tokens_ent_path = self._get_tokens_path(bert_refine_triples_ids, bert_refine_triples_with_cls_ids, entity_ids, entpair2tripleid, input_ids, tokens_to_triple, tokenizer, ents_path_dict)
    ...
    def _get_tokens_path(self, refine_triples_ids, refine_triples_with_cls_ids, entity_ids, entpair2tripleid,
        ...
        # cls_ids = []
        # for ent in entity_ids.keys():
        #     cls_ids.append((len(refine_triples_with_cls_ids), 2))
        #     refine_triples_with_cls_ids.append([str_to_int_list(ent), [cls_id], [cls_id]]) # 额外添加新的CLS三元组
        # entity_ids[int_list_to_str([cls_id])] = cls_ids
        # for ent in entity_ids.keys():
        #     entity_ids[ent].append((len(refine_triples_with_cls_ids), 0))  # 每个实体添加与cls关联
        ...
        for token_idx, tokeni2triple in enumerate(tokens_to_triple):
            if tokeni2triple != [[-1, -1]]: # 说明当前的token出现在三元组内，则该token参与attention计算
                for idx, tokenj2triple in enumerate(tokens_to_triple[token_idx:]):
                    ...
                    if tokenj2triple != [[-1, -1]]: # 说明此时两个token都对应图中的三元组，一定存在路径，mask为1
                        ...
                        start = int_list_to_str(refine_triples_with_cls_ids[tokeni2triple[0][0]][posi]) # 路径的起始实体(实体包含的word piece token id序列并以下划线相隔， 例如343_54_3232)
                        end = int_list_to_str(refine_triples_with_cls_ids[tokenj2triple[0][0]][posj]) # 路径的终点实体
                        if (start, end) in ents_path_dict.keys():
                            ent_path = ents_path_dict[(start, end)][0]
                            rel_path = ents_path_dict[(start, end)][1]
                        elif (end, start) in ents_path_dict.keys():
                            ent_path = ents_path_dict[(end, start)][0]
                            rel_path = ents_path_dict[(end, start)][1]
                        else:
        ...
        '''
        return tokens_rel_path, tokens_ent_path
    ...
    def convert_path_dict_to_tensor(self, args, input_rel_path_dict, input_ent_path_dict):
        ...
        input_ent_path_length = np.zeros([len(input_rel_path_dict), args.max_seq_length, args.max_seq_length]).tolist()

        for example in range(len(input_ent_path_dict)): # 遍历一个batch内的所有样本
            for (tgt, src) in input_ent_path_dict[example].keys():
                rel_path = input_rel_path_dict[(tgt, src)][:args.path_length]
                ent_path = input_ent_path_dict[(tgt, src)][:args.path_length]
                reverse_rel_path = (copy.deepcopy(rel_path)).reverse()
                reverse_ent_path = (copy.deepcopy(ent_path)).reverse()
        ...
        return torch.tensor(input_rel_path, dtype=torch.long), torch.tensor(input_ent_path, dtype=torch.long), input_rel_path_length, input_ent_path_length
