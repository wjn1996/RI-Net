...
from DataLoader import SquadExample, SquadLoader, ModelTrainInput
...
def main():
    ...
    TrainLoss = []
    if args.do_train:
        # 使用多进程实现处理结构化知识点额提取处理工作
        process_map_file = os.path.join(args.squad_data_dir, 'squad_process_map.npy') # 每个process处理的样本编号与process id的映射
        if not os.path.exists(process_map_file):
            # 如果已存在这个映射文件，说明已经进行过多进程处理并保存了相应的文件 否则重新进行多进程处理
            ...
            logger.info("***** Running Multi Processing *****")
            multi_process_refiner(train_features, squadloader)
        process_map = np.load(process_map_file, allow_pickle=True)[()]
        n_cpu = process_map['n_cpu']
        chunk_num = process_map['chunk_num']
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for cpuid in range(n_cpu):
                logger.info("Reading the {}-th of CPU (has {} features)".format(str(cpuid), str(chunk_num[cpuid])))
                # 读取每个CPU处理的数据
                file_name = os.path.join(args.squad_data_dir, 'squad_model_input_{}.pkl'.format(str(cpuid)))
                with open(cached_train_features_file, "rb") as reader:
                    modelTrainInut = pickle.load(reader)

                all_input_ids = torch.tensor([f.input_ids for f in modelTrainInut], dtype=torch.long)  # 输入word piece token序列
                all_input_mask = torch.tensor([f.input_mask for f in modelTrainInut], dtype=torch.long)  # mask（没课token对应是否是padding）
                all_segment_ids = torch.tensor([f.segment_ids for f in modelTrainInut], dtype=torch.long)  # segment编号
                all_input_kg_rel_path = torch.tensor([f.input_kg_rel_path for f in modelTrainInut], dtype=torch.long)  # answer起始位置
                all_input_kg_rel_path_length = torch.tensor([f.input_kg_rel_path_length for f in modelTrainInut], dtype=torch.long)  # answer起始位置
                all_input_token_to_entity = torch.tensor([f.input_token_to_entity for f in modelTrainInut], dtype=torch.long)  # answer起始位置
                all_input_path_mask = torch.tensor([f.input_path_mask for f in modelTrainInut], dtype=torch.long)  # answer起始位置
                all_start_positions = torch.tensor([f.start_position for f in modelTrainInut], dtype=torch.long)  # answer起始位置
                all_end_positions = torch.tensor([f.end_position for f in modelTrainInut], dtype=torch.long)  # answer终止位置
                all_is_impossibles = torch.tensor([int(f.is_impossible) for f in modelTrainInut], dtype=torch.long)  # 是否不可回答

                # dataset类自动加载为训练集对象
                train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_input_kg_rel_path,
                                           all_input_kg_rel_path_length, all_input_token_to_entity, all_input_path_mask,
                ...
                for _, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ncols=50)):
                    step += 1
                    if n_gpu == 1:
                        batch = tuple(t.to(device) for t in batch)  # multi-gpu does scattering it-self
                    input_ids, input_mask, segment_ids, input_kg_rel_path, input_kg_rel_path_length, \
                    input_token_to_entity, input_path_mask, start_positions, end_positions, \
                    is_impossibles, example_indices = batch

                    #### stage2：前向传播计算
                    loss = model(input_ids, segment_ids, input_mask, input_kg_rel_path,
                                 input_kg_rel_path_length, input_token_to_entity,
                                 input_path_mask, start_positions, end_positions, is_impossibles)
                    if n_gpu > 1:
...
def multi_process_refiner(features, squadloader):
    # 多进程将结构化知识进行预处理
    ...
    for i in range(0, n_cpu):
        ...
        if i < n_cpu - 1:
            ...
            max_i = n
        digits = [features[min_i:max_i], str(i)]
        chunk_num[i] = max_i - min_i
        procs.append(multiprocessing.Process(target=refiner, args=(digits, squadloader, "parallel")))

    info = {
        "n_cpu": n_cpu, # CPU核数
        "feature_num": n, # 总的样本数
        "chunk_num": chunk_num # 每个CPU所处理的样本数
    }
    file_name = os.path.join(args.squad_data_dir, 'squad_process_map.npy')
...
def refiner(digits, squadloader, fold="1by1"):
    features = digits[0]
    cpuid = digits[1]
    file_name = os.path.join(args.squad_data_dir, 'squad_model_input_{}.pkl'.format(str(cpuid)))
    feature_num = len(features)
    all_input_ids = [f.input_ids for f in features]  # 输入word piece token序列
    all_input_mask = [f.input_mask for f in features]  # mask（没课token对应是否是padding）
    all_segment_ids = [f.segment_ids for f in features]  # segment编号
    all_start_positions = [f.start_position for f in features]  # answer起始位置
    all_end_positions = [f.end_position for f in features]  # answer终止位置
    all_is_impossibles = [int(f.is_impossible) for f in features]  # 是否不可回答

    ### 以下为为了提炼结构化知识（token与token之间的推理路径）的中间变量
    all_bert_refine_triples_ids = [f.bert_refine_triples_ids for f in features]
    all_bert_refine_triples_with_cls_ids = [f.bert_refine_triples_with_cls_ids for f in features]
    all_entity_ids = [f.entity_ids for f in features]
    all_entpair2tripleid = [f.entpair2tripleid for f in features]
    all_tokens_to_triple = [f.tokens_to_triple for f in features]
    ### 以上为为了提炼结构化知识（token与token之间的推理路径）的中间变量

    #### stage1: Refining Stage
    cls_id = 101
    # 对batch内的每个样本，首先提炼结构化知识，并基于此实现计算每两个结点之间的最短推理路径
    logger.info("CPU {} : step1---find_shortest_path_and_convert_to_tensor".format(str(cpuid)))
    input_kg_rel_path, input_kg_rel_path_length, \
    input_kg_path_mask, entity2id_list = \
        squadloader.find_shortest_path_and_convert_to_tensor(all_bert_refine_triples_ids,
                                                             all_bert_refine_triples_with_cls_ids,
                                                             all_entity_ids, all_entpair2tripleid, cls_id)
    logger.info("CPU {} : step2---get_tokens_to_entity".format(str(cpuid)))
    input_token_to_entity, input_path_mask = squadloader.get_tokens_to_entity(all_bert_refine_triples_with_cls_ids,
                                                                              all_tokens_to_triple, entity2id_list)
    model_inputs = []
    for i in range(feature_num):
        model_input = ModelTrainInput(
            input_ids = all_input_ids[i],
            input_mask = all_input_mask[i],
            segment_ids = all_segment_ids[i],
            input_kg_rel_path = input_kg_rel_path[i],
            input_kg_rel_path_length = input_kg_rel_path_length[i],
            input_token_to_entity = input_token_to_entity[i],
            input_path_mask = input_path_mask[i],
            start_position = all_start_positions[i],
            end_position = all_end_positions[i],
            is_impossible = all_is_impossibles[i],
        )
        model_inputs.append(model_input)
    logger.info("CPU {} : step3---saving features to squad_model_input_{}.pkl".format(str(cpuid), str(cpuid)))
    with open(file_name, "wb") as writer:
        pickle.dump(model_inputs, writer)
